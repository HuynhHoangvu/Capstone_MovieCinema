...
const Seat = ({ seat, isSelecting, onSelect }) => {
  let className =
    "w-6 h-6 m-0.5 md:w-8 md:h-8 md:m-1 flex items-center justify-center text-xs font-semibold rounded transition-colors duration-150";
  ...
  if (isSelecting) {
    // Ghế đang chọn 
    className +=
      " bg-green-500 border border-green-600 text-white cursor-pointer";
  } else if (isVIP) {
    // Ghế VIP 
...
const BookingRoom = () => {
  ...
  const { loading, data, error, bookingLoading, bookingError } = useSelector(
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const errorMessage =
        err.content || err.message || "Đặt vé thất bại. Vui lòng thử lại.";
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-gray-900 text-white p-8 text-center text-xl">
        Đang tải thông tin phòng vé...
      </div>
    );
  if (error)
    return (
      <div className="min-h-screen bg-gray-900 text-red-500 p-8 text-center text-xl">
        Lỗi: {error.message || "Không thể tải dữ liệu phòng vé."}
      </div>
    );
  if (!thongTinPhim)
    return (
      <div className="min-h-screen bg-gray-900 text-white p-8 text-center text-xl">
        Không tìm thấy thông tin phim/lịch chiếu.
      </div>
    );
        ...
        <div className="flex items-center space-x-2">
          ...
          <div
            className={`text-lg font-bold ${
              countdown <= 60 ? "text-red-500 animate-pulse" : "text-orange-400"
            }`}
          >
          ...
          <div className="flex flex-col items-center overflow-x-auto p-2">
            {Object.entries(seatsByRow).map(([row, seats]) => (
              <div key={row} className="flex items-center mb-1">
                <span className="w-6 text-center mr-2 font-bold text-gray-400">
                  {row}
                </span>
                <div className="flex">
                  {seats.map((seat) => (
                    <Seat
                      key={seat.maGhe}
                      seat={seat}
                      isSelecting={selectedSeats.some(
        ...
        <div className="p-4 bg-gray-800 rounded-lg shadow-xl self-start lg:col-span-1">
          ...
          {bookingError && (
            <p className="text-red-500 text-xs mt-2 text-center">
              {bookingError.message || "Lỗi đặt vé: Vui lòng thử lại."}
            </p>
          )}
        </div>
      </div>
